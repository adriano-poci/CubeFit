# kaczmarz_solver_cchunk_mp.py
# Modestly-MP, chunk-friendly solver aligned to /HyperCube/models chunking.
# External contract matches your other solvers:
#   - x is 1-D (length C*P) in/out
#   - warm start handled by caller; we accept x0
#   - orbit_weights is accepted (optional); free fit if None

from __future__ import annotations, print_function

import os, sys
import math
import time
from dataclasses import dataclass
from typing import Optional, Tuple, List
from contextlib import contextmanager
from threadpoolctl import threadpool_limits

import multiprocessing as mp
import numpy as np
import h5py
from tqdm import tqdm

from CubeFit.hdf5_manager import open_h5
from CubeFit.hypercube_builder import read_global_column_energy
from CubeFit.cube_utils import read_lambda_weights, ensure_lambda_weights,\
    _get_mask

# ----------------------------- Config ---------------------------------

@dataclass
class MPConfig:
    epochs: int = 1
    lr: float = 0.25
    project_nonneg: bool = True
    processes: int = 2              # modest parallelism (2–4 recommended)
    blas_threads: int = 8           # per-process BLAS threads
    apply_mask: bool = True
    # HDF5 *dataset* chunk cache (not RDCC): keep local & harmless
    dset_slots: int = 1_000_003
    dset_bytes: int = 256 * 1024**2
    dset_w0: float = 0.90

# ------------------------------------------------------------------------------

def _assert_norm_mode(h5_path: str, expect: Optional[str] = None) -> str:
    r"""
    Read /HyperCube/norm.mode and optionally assert a specific mode.

    Parameters
    ----------
    h5_path : str
        Path to HDF5 file.
    expect : str, optional
        If given ('data' or 'model'), raise if the stored mode differs.

    Returns
    -------
    str
        The stored normalization mode ('data' or 'model').

    Exceptions
    ----------
    RuntimeError
        If the attribute is missing or does not match `expect`.
    """
    with open_h5(h5_path, role="reader") as f:
        g = f["/HyperCube"]
        mode = str(g.attrs.get("norm.mode", "model")).lower()
    if expect and mode != expect:
        raise RuntimeError(
            f"Hypercube is in norm.mode='{mode}', but solver expects "
            f"'{expect}'. Convert first with convert_hypercube_norm(...)."
        )
    if mode not in ("data", "model"):
        raise RuntimeError(f"Unexpected norm.mode='{mode}'.")
    return mode

# ------------------------------------------------------------------------------

def _read_mask_and_lambda(h5_path: str,
                          use_mask: bool,
                          use_lambda: bool,
                          lam_dset: str = "/HyperCube/lambda_weights"
                          ) -> Tuple[Optional[np.ndarray], np.ndarray]:
    r"""
    Load wavelength mask and λ-weights like nnls_patch.

    Returns
    -------
    keep_idx : ndarray or None
        Indices of wavelengths to keep (mask applied), or None.
    w_lam : ndarray (L,)
        Per-wavelength weights; 1s if not present/disabled.

    Notes
    -----
    Follows nnls_patch: if no /Mask exists or `use_mask=False`, keep_idx
    is None. If no λ-weights exist or `use_lambda=False`, w_lam is ones.
    """
    with open_h5(h5_path, role="reader") as f:
        L = int(f["/DataCube"].shape[1])
        if use_mask and "/Mask" in f:
            m = np.asarray(f["/Mask"][...], bool).ravel()
            keep_idx = np.flatnonzero(m) if (m.size == L and np.any(m)) \
                       else None
        else:
            keep_idx = None

        if use_lambda and lam_dset in f:
            w_full = np.asarray(f[lam_dset][...], np.float64).ravel()
            if w_full.size != L:
                raise RuntimeError(
                    f"{lam_dset} has shape {w_full.shape}, expected (L,)={L}."
                )
        else:
            w_full = np.ones(L, dtype=np.float64)

    return keep_idx, w_full

# ------------------------------------------------------------------------------

def _row_weights_for_spaxels(h5_path: str,
                             s_idx: np.ndarray,
                             keep_idx: Optional[np.ndarray],
                             w_lam: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    r"""
    Build the **row weights** and RHS y, exactly like nnls_patch.

    Returns
    -------
    y : ndarray (S_sel * Lk,)
        RHS stacked [s0 λ0..λk-1, s1 λ0.., ...] with NaNs→0.
    sqrt_w_rows : ndarray (S_sel * Lk,)
        Per-row weights = sqrt(w_lam) * 1[finite(data)].

    Notes
    -----
    Identical stacking order and finite-pixel handling as nnls_patch to
    guarantee the same absolute scale and conditioning.
    """
    s_idx = np.asarray(s_idx, np.int64)
    with open_h5(h5_path, role="reader") as f:
        DC = f["/DataCube"]
        L = int(DC.shape[1])
        Lk = int(keep_idx.size) if keep_idx is not None else L
        rows = int(s_idx.size * Lk)
        y = np.empty(rows, dtype=np.float64)
        sqrt_w_rows = np.empty(rows, dtype=np.float64)
        pos = 0
        for s in s_idx:
            spec = np.asarray(DC[int(s), :], np.float64)
            if keep_idx is not None:
                spec = spec[keep_idx]
                wl = w_lam[keep_idx]
            else:
                wl = w_lam
            good = np.isfinite(spec)
            y[pos:pos+Lk] = np.where(good, spec, 0.0)
            sqrt_w_rows[pos:pos+Lk] = np.sqrt(wl) * good.astype(np.float64)
            pos += Lk
    return y, sqrt_w_rows

# ------------------------------------------------------------------------------

def _apply_row_weight_to_row(a_row: np.ndarray,
                             b_val: float,
                             w: float) -> Tuple[np.ndarray, float, float]:
    r"""
    Apply per-row weight `w` to a single Kaczmarz update row.

    Returns
    -------
    a_w : ndarray
        Weighted row: a_w = w * a_row
    b_w : float
        Weighted RHS: b_w = w * b_val
    denom : float
        Squared norm used in the update: ||a_w||^2

    Notes
    -----
    Your Kaczmarz update should use:
        x ← max(0, x + (b_w − a_w·x) / denom * a_w)
    """
    if w != 1.0:
        a_row = a_row * w
        b_val = float(b_val * w)
    denom = float(a_row @ a_row)
    return a_row, b_val, denom

# ------------------------------------------------------------------------------

def compute_weighted_col_norms(h5_path: str,
                               s_idx: np.ndarray,
                               picks: Iterable[Tuple[int, int]],
                               keep_idx: Optional[np.ndarray],
                               sqrt_w_rows: np.ndarray) -> np.ndarray:
    r"""
    Compute column 2-norms of the **weighted** system used by Kaczmarz.

    Parameters
    ----------
    h5_path : str
        HDF5 file path.
    s_idx : ndarray of int
        Spaxel indices in the same order used to stack rows.
    picks : iterable of (c, p)
        Columns to include (component, population).
    keep_idx : ndarray or None
        Wavelength mask indices. If None, all L rows are used.
    sqrt_w_rows : ndarray
        Per-row weights as returned by `_row_weights_for_spaxels`.

    Returns
    -------
    col_norm : ndarray (K,)
        Weighted 2-norms for each selected column, in the same order
        as `picks`.

    Notes
    -----
    This reproduces the **exact** norms that `nnls_patch` sees after the
    `_weighted_system` call. You may divide each column by these and
    later map back `x = z / col_norm`.
    """
    picks = list(picks)
    K = int(len(picks))
    col_norm2 = np.zeros(K, dtype=np.float64)

    with open_h5(h5_path, role="reader") as f:
        M = f["/HyperCube/models"]  # (S, C, P, L)
        L = int(M.shape[3])
        Lk = int(keep_idx.size) if keep_idx is not None else L
        assert sqrt_w_rows.size == int(s_idx.size * Lk)

        for j, (c, p) in enumerate(picks):
            pos = 0
            acc = 0.0
            for s in s_idx:
                col = np.asarray(M[int(s), int(c), int(p), :], np.float64)
                if keep_idx is not None:
                    col = col[keep_idx]
                good_w = sqrt_w_rows[pos:pos+Lk]
                # rows with 0 weight contribute nothing
                v = col * good_w
                acc += float(v @ v)
                pos += Lk
            col_norm2[j] = acc

    # Avoid zeros to keep divisions safe (consistent with nnls_patch)
    col_norm = np.sqrt(np.maximum(col_norm2, 1.0))
    return col_norm

# ---- tiny top-level helpers (must be picklable!) -----------------
def _pool_ping() -> int:
    # trivial pure-Python function; safe to pickle
    return 1

def _pool_ok(pool, timeout: float = 5.0) -> bool:
    """
    Returns True if a trivial task round-trips within `timeout`.
    If it times out or raises, the pool is considered unhealthy.
    """
    try:
        res = pool.apply_async(_pool_ping)
        return res.get(timeout=timeout) == 1
    except Exception:
        return False

# @contextmanager
# def _single_blas_thread():
#     """
#     Temporarily force BLAS (MKL/OpenBLAS/Accelerate) to 1 thread in *this*
#     process. Outside the context, settings revert automatically.
#     """
#     with threadpool_limits(1):
#         yield

def _worker_init(blas_threads: int) -> None:
    os.environ["OMP_NUM_THREADS"] = str(blas_threads)
    os.environ["OPENBLAS_NUM_THREADS"] = str(blas_threads)
    os.environ["MKL_NUM_THREADS"] = str(blas_threads)
    os.environ["NUMEXPR_NUM_THREADS"] = str(np.max((1, blas_threads // 2)))
    os.environ.setdefault("KMP_INIT_AT_FORK", "FALSE")

def _worker_tile_job_with_R(args):
    """
    Worker for one S-tile and a contiguous c-band. Robust version that
    protects against NaN/Inf, near-zero columns, and oscillations while
    preserving physical zeros in A (no normalization of inputs).

    Changes vs. original:
      * Sanitize A and R in-place: non-finite -> 0.
      * Freeze columns with near-zero total energy on the tile.
      * Diagonal preconditioning: dx_p = lr * g_p / sum(A[:,p,:]^2).
      * Trust-region on Δy: ||A Δx|| <= τ ||R|| (τ ~ 0.7 by default).
      * Backtracking on α for monotone RMSE drop.
      * Blend local denominators with global column energy (β * E_global).
      * Return per-band global update energy for a global step cap.
      * Propose top-(global_cp, score) columns per band for tile-local NNLS.
    """
    (h5_path, s0, s1, keep_idx,
     c_start, c_stop, x_band, lr, project_nonneg, R, w_band,
     dset_slots, dset_bytes, dset_w0,
     E_global_band, beta_blend,
     w_lam_sqrt) = args

    bt_steps   = int(os.environ.get("CUBEFIT_BT_STEPS", "3"))
    bt_factor  = float(os.environ.get("CUBEFIT_BT_FACTOR", "0.5"))
    tau_trust  = float(os.environ.get("CUBEFIT_TRUST_TAU", "0.7"))
    eps        = float(os.environ.get("CUBEFIT_EPS", "1e-12"))
    rel_zero   = float(os.environ.get("CUBEFIT_ZERO_COL_REL", "1e-12"))
    abs_zero   = float(os.environ.get("CUBEFIT_ZERO_COL_ABS", "1e-24"))
    dbg        = os.environ.get("CUBEFIT_DEBUG_SAFE", "0") == "1"
    nnls_prop_per_band = int(os.environ.get("CUBEFIT_NNLS_PROP_PER_BAND", "6"))

    bt_steps = np.max((0, bt_steps))
    beta_blend = float(beta_blend)

    def _finite_inplace(arr, name, stats_dict):
        bad = ~np.isfinite(arr)
        if bad.any():
            arr[bad] = 0.0
            if stats_dict is not None:
                stats_dict[name] = int(np.sum(bad))

    with open_h5(h5_path, role="reader") as f:
        M = f["/HyperCube/models"]  # (S,C,P,L)
        try:
            M.id.set_chunk_cache(dset_slots, dset_bytes, dset_w0)
        except Exception:
            pass

        Sblk = int(s1 - s0)
        P    = int(M.shape[2])
        L    = int(M.shape[3])
        Lk   = L if keep_idx is None else int(keep_idx.size)

        R = np.asarray(R, dtype=np.float64, order="C")  # (Sblk,Lk)
        stats = {}
        _finite_inplace(R, "R_nonfinite", stats)

        # Weighted residual (√w * R) if provided
        if w_lam_sqrt is not None:
            wvec = np.asarray(w_lam_sqrt, dtype=np.float64).ravel()
            if wvec.size != R.shape[1]:
                raise RuntimeError(f"w_lam_sqrt length {wvec.size} != Lk={R.shape[1]}")
            Rw = R * wvec[None, :]
        else:
            Rw = R

        rmse_before_tile = float(np.sqrt(np.mean(R * R)))
        # unweighted for reporting

        R_delta = np.zeros((Sblk, Lk), dtype=np.float64)
        dx_list = []
        nnls_candidates = []

        for bi, c in enumerate(range(c_start, c_stop)):
            A = np.asarray(M[s0:s1, c, :, :], dtype=np.float32, order="C")
            if keep_idx is not None:
                A = A[:, :, keep_idx]  # (Sblk,P,Lk)
            _finite_inplace(A, f"A_nonfinite_c{c}", stats)

            # ---- λ-weighting (feature emphasis) for gradient/denom ONLY
            # Reuse tile-wide Rw computed above; only weight A here.
            if w_lam_sqrt is not None:
                A_w = A * w_lam_sqrt[None, None, :]   # (Sblk,P,Lk)
            else:
                A_w = A

            # ---- Gradient using weighted views
            g = np.zeros((P,), dtype=np.float64)
            for s in range(Sblk):
                g += A_w[s, :, :].astype(np.float64, copy=False) @ Rw[s, :]

            # ---- Column energy denom using weighted A
            col_denom = np.sum(np.square(A_w, dtype=np.float64), axis=(0, 2))  # (P,)

            # Adaptive threshold and freezing (on the same weighted energy)
            if np.any(col_denom > 0):
                med_energy = float(np.median(col_denom[col_denom > 0]))
            else:
                med_energy = 0.0
            tiny_col = np.max((abs_zero, rel_zero * med_energy))
            freeze = col_denom <= tiny_col
            if freeze.any():
                g[freeze] = 0.0
                col_denom = np.where(freeze, np.inf, col_denom)

            # blend with global column energy for this band
            Eg_row = None
            if E_global_band is not None:
                Eg_row = np.asarray(E_global_band[bi, :], dtype=np.float64)  # (P,)
                if Eg_row.size == col_denom.size:
                    col_denom = np.maximum(col_denom, beta_blend * Eg_row)

            if dbg:
                stats[f"frozen_cols_c{c}"] = int(np.count_nonzero(freeze))

            invD = 1.0 / np.maximum(col_denom, eps)
            dx_c = float(lr) * (g * invD)  # (P,)

            if w_band is not None:
                dx_c *= float(w_band[bi])

            # NNLS candidate proposal (optional)
            if nnls_prop_per_band > 0:
                score = np.abs(g) / (np.sqrt(np.maximum(col_denom, eps)) + eps)
                if np.any(freeze):
                    score[freeze] = 0.0
                k_keep = int(np.min((nnls_prop_per_band, int(np.count_nonzero(score > 0.0)))))
                if k_keep > 0:
                    top_p = np.argpartition(score, -k_keep)[-k_keep:]
                    ordr  = np.argsort(score[top_p])[::-1]
                    top_p = top_p[ordr]
                    top_global = (int(c) * P + top_p).astype(np.int64)
                    for j in range(top_p.size):
                        nnls_candidates.append((int(top_global[j]), float(score[top_p[j]])))

            # ---- ΔR (UNweighted) for α=1
            R_delta_band = np.zeros((Sblk, Lk), dtype=np.float64)
            for s in range(Sblk):
                R_delta_band[s, :] -= (A[s, :, :].astype(np.float64, copy=False).T @ dx_c)

            # ---- Weighted trust/backtracking: use Rw, and ΔRw = √w ⊙ ΔR
            if w_lam_sqrt is not None:
                Rw_delta_band = R_delta_band * wvec[None, :]
            else:
                Rw_delta_band = R_delta_band

            rd_norm = float(np.linalg.norm(Rw_delta_band))
            if rd_norm > 0.0:
                r_norm = float(np.linalg.norm(Rw))
                alpha_max = np.min((1.0, (tau_trust * r_norm) / rd_norm))
            else:
                alpha_max = 1.0

            alpha = alpha_max
            rmse_before_w = float(np.sqrt(np.mean(Rw * Rw)))
            rmse_after_w  = float(np.sqrt(np.mean((Rw + alpha * Rw_delta_band) ** 2)))
            if not (rmse_after_w < rmse_before_w):
                a = alpha
                for _ in range(bt_steps):
                    a *= bt_factor
                    if a <= 0.0:
                        break
                    rmse_after_w = float(np.sqrt(np.mean((Rw + a * Rw_delta_band) ** 2)))
                    if rmse_after_w < rmse_before_w:
                        alpha = a
                        break
                else:
                    alpha = a

            # finalize dx; if we clip, recompute ΔR and ΔRw
            dx_c = alpha * dx_c
            if project_nonneg:
                over_neg = dx_c < -x_band[bi, :]
                if np.any(over_neg):
                    dx_c[over_neg] = -x_band[bi, :][over_neg]
                    # recompute ΔR for the *final* dx_c
                    R_delta_band.fill(0.0)
                    for s in range(Sblk):
                        R_delta_band[s, :] -= (A[s, :, :].astype(np.float64, copy=False).T @ dx_c)
                    if w_lam_sqrt is not None:
                        Rw_delta_band = R_delta_band * wvec[None, :]
            else:
                if alpha != 1.0:
                    R_delta_band  *= alpha
                    Rw_delta_band *= alpha

            # per-band global update-energy estimate (Eg_row blended outside)
            if Eg_row is not None:
                upd_energy_sq = float(np.sum((dx_c.astype(np.float64) ** 2) * Eg_row))
            else:
                upd_energy_sq = 0.0

            # accumulate (return UNweighted ΔR)
            R_delta += R_delta_band
            dx_list.append((c, dx_c, upd_energy_sq))

        if dbg and stats:
            try:
                print("[SAFE]", {k: int(v) for k, v in stats.items()})
            except Exception:
                pass

        return R_delta, dx_list, nnls_candidates

# ---------------------------- Coordinator ------------------------------

def solve_global_kaczmarz_cchunk_mp(
    h5_path: str,
    cfg: MPConfig,
    *,
    orbit_weights: Optional[np.ndarray] = None,   # (C,) or None
    x0: Optional[np.ndarray] = None, # 1-D (C*P,) or None
    tracker: Optional[object] = None,  # FitTracker or None
) -> tuple[np.ndarray, dict]:

    def _set_chunk_cache(dset, cfg_obj):
        try:
            dset.id.set_chunk_cache(cfg_obj.dset_slots, cfg_obj.dset_bytes, cfg_obj.dset_w0)
        except Exception:
            pass

    # ------------------- read dims, chunks, mask -------------------
    t0 = time.perf_counter()
    with open_h5(h5_path, role="reader") as f:
        DC = f["/DataCube"]           # (S,L) float64
        M  = f["/HyperCube/models"]   # (S,C,P,L) float32
        _set_chunk_cache(M, cfg)

        S, L = map(int, DC.shape)
        _, C, P, Lm = map(int, M.shape)
        if Lm != L:
            raise RuntimeError(f"L mismatch: models L={Lm} vs data L={L}.")

        mask = _get_mask(f) if cfg.apply_mask else None
        keep_idx = np.flatnonzero(mask) if mask is not None else None
        Lk = int(keep_idx.size) if keep_idx is not None else L

        # ------------------- λ-weights (feature emphasis) -------------------
        lamw_enable = os.environ.get("CUBEFIT_LAMBDA_WEIGHTS_ENABLE", "1").lower() not in ("0","false","no","off")
        lamw_dset   = os.environ.get("CUBEFIT_LAMBDA_WEIGHTS_DSET", "/HyperCube/lambda_weights")
        lamw_floor  = float(os.environ.get("CUBEFIT_LAMBDA_MIN_W", "1e-6"))
        lamw_auto   = os.environ.get("CUBEFIT_LAMBDA_WEIGHTS_AUTO", "1").lower() not in ("0","false","no","off")

        if lamw_enable:
            print(f"[Kaczmarz-MP] Reading λ-weights from '{lamw_dset}' (floor={lamw_floor}, auto={lamw_auto})...", flush=True)
            try:
                w_full = read_lambda_weights(h5_path, dset_name=lamw_dset, floor=lamw_floor)
            except Exception:
                if lamw_auto:
                    w_full = ensure_lambda_weights(h5_path, dset_name=lamw_dset)
                else:
                    w_full = np.ones(L, dtype=np.float64)
            print(f"[Kaczmarz-MP] λ-weights min={float(np.min(w_full)):.3e}, max={float(np.max(w_full)):.3e}, mean={float(np.mean(w_full)):.3e}", flush=True)
            if keep_idx is not None:
                w_lam = w_full[keep_idx]
            else:
                w_lam = w_full
            # we use sqrt(w) on both A and R
            w_lam_sqrt = np.sqrt(np.clip(w_lam, lamw_floor, None)).astype(np.float64, order="C")
        else:
            w_lam_sqrt = None

        s_tile = int(M.chunks[0]) if (M.chunks and M.chunks[0] > 0) else 128
        s_ranges = [(s0, np.min((S, s0 + s_tile))) for s0 in range(0, S, s_tile)]

        # for ordering + compute global ||Y|| while we touch DC
        norms = []
        Y_glob_norm2 = 0.0
        for (ss0, ss1) in s_ranges:
            Yt = np.asarray(DC[ss0:ss1, :], dtype=np.float64)
            if keep_idx is not None:
                Yt = Yt[:, keep_idx]
            norms.append(float(np.linalg.norm(Yt)))
            Y_glob_norm2 += float(np.sum(Yt * Yt))
    s_ranges = [sr for _, sr in sorted(zip(norms, s_ranges), key=lambda t: -t[0])]
    Y_glob_norm = float(np.sqrt(Y_glob_norm2))

    # ------------------- global column energy & knobs -------------------
    E_global = read_global_column_energy(h5_path)  # (C,P) float64
    tau_global = float(os.environ.get("CUBEFIT_GLOBAL_TAU", "0.5"))
    beta_blend = float(os.environ.get("CUBEFIT_GLOBAL_ENERGY_BLEND", "1e-3"))

    # ------------------- x init -------------------
    if x0 is None:
        x_CP = np.zeros((C, P), dtype=np.float64)
    else:
        x0 = np.asarray(x0, np.float64).ravel(order="C")
        if x0.size != C * P:
            raise ValueError(f"x0 length {x0.size} != C*P={C*P}.")
        x_CP = x0.reshape(C, P)
    # --- symmetry breaking warm-start (tiny, orthogonal, nonneg)
    sym_eps  = float(os.environ.get("CUBEFIT_SYMBREAK_EPS", "1e-6")) # ~1e-6–1e-4
    sym_mode = os.environ.get("CUBEFIT_SYMBREAK_MODE", "qr").lower()
    # "qr"|"rand"|"off"
    if sym_eps > 0.0 and sym_mode != "off" and (x0 is None or np.count_nonzero(x_CP) == 0):
        rng = np.random.default_rng(int(os.environ.get("CUBEFIT_SEED", "12345")))
        if sym_mode == "qr":
            # Build C row-orthogonal directions in ℝ^P and add tiny non-negative seed
            Rmat = rng.standard_normal((C, P))
            # QR on (P × C), take first C columns → (P,C), then transpose → (C,P)
            Q, _ = np.linalg.qr(Rmat.T, mode="reduced")
            Q = Q[:, :C].T  # (C,P)
            x_CP += sym_eps * np.abs(Q)
        else:
            x_CP += sym_eps * rng.random((C, P))
        if cfg.project_nonneg:
            np.maximum(x_CP, 0.0, out=x_CP)


    # ------------------- ratio penalty setup -------------------
    have_ratio = False
    w_full = None
    if orbit_weights is not None:
        w = np.asarray(orbit_weights, dtype=np.float64).ravel()
        if w.size != C:
            raise ValueError(f"orbit_weights length {w.size} != C={C}.")
        w_sum = float(np.sum(w)) if w.size else 0.0
        w_c = (w / w_sum) if w_sum > 0 else np.ones_like(w) / np.max((1, C))
        have_ratio = True
        _ratio_eta = 0.02
        _ratio_prob = 0.02
        _ratio_batch = 2
        _ratio_minw = 1e-4
        rng = np.random.default_rng()

        w_full = np.asarray(orbit_weights, dtype=np.float64).ravel(order="C")
        if w_full.size != C:
            raise ValueError(f"orbit_weights length {w_full.size} != C={C}.")
        m = float(np.mean(w_full)) or 1.0
        w_full = w_full / m

        def _ratio_update_in_place(x_mat: np.ndarray) -> None:
            s = x_mat.sum(axis=1)
            eps = 1e-12
            active = (w_c >= _ratio_minw) | (s > 0)
            if not np.any(active):
                return
            a = int(np.argmax(w_c * active))
            sa = float(np.max((s[a], eps)))
            wa = float(np.max((w_c[a], eps)))
            pool = np.flatnonzero(active & (np.arange(C) != a))
            if pool.size == 0:
                return
            sel = pool[rng.choice(pool.size, size=np.min((_ratio_batch, pool.size)), replace=False)]
            if sel.size > 1 and _ratio_prob < 1.0:
                keep_mask = rng.random(sel.size) < _ratio_prob
                sel = sel[keep_mask]
                if sel.size == 0:
                    return
            for c in sel:
                sc = float(np.max((s[c], eps)))
                rc = float(np.max((w_c[c], eps)))
                e = math.log((sc/sa) / (rc/wa))
                if e == 0.0:
                    continue
                delta_sc = -_ratio_eta * e * sc
                delta_sa = +_ratio_eta * e * sa
                pc = x_mat[c, :] / sc
                pa = x_mat[a, :] / sa
                if not np.all(np.isfinite(pc)):
                    pc = np.full(P, 1.0 / P, dtype=np.float64)
                if not np.all(np.isfinite(pa)):
                    pa = np.full(P, 1.0 / P, dtype=np.float64)
                x_mat[c, :] += delta_sc * pc
                x_mat[a, :] += delta_sa * pa
            if cfg.project_nonneg:
                np.maximum(x_mat, 0.0, out=x_mat)

    # ------------------- bands & pool -------------------
    nprocs = np.max((1, int(cfg.processes)))
    band_size = math.ceil(C / nprocs)
    bands: List[Tuple[int, int]] = []
    c0 = 0
    for _ in range(nprocs):
        c1 = np.min((C, c0 + band_size))
        if c1 > c0:
            bands.append((c0, c1))
        c0 = c1
    nprocs = len(bands)

    print("[Kaczmarz-MP] Spinning up workers…", file=sys.__stdout__, flush=True)

    ctx_name = os.environ.get("CUBEFIT_MP_CTX", "forkserver")
    # safer than plain fork
    try:
        ctx = mp.get_context(ctx_name)
    except ValueError:
        ctx = mp.get_context("spawn")  # fallback

    # Pre-set anti-deadlock knobs in the *parent* too (not only in _worker_init)
    os.environ.setdefault("KMP_INIT_AT_FORK", "FALSE")
    os.environ.setdefault("OMP_NUM_THREADS", str(cfg.blas_threads))
    os.environ.setdefault("OPENBLAS_NUM_THREADS", str(cfg.blas_threads))
    os.environ.setdefault("MKL_NUM_THREADS", str(cfg.blas_threads))
    # --- pool knobs
    max_tasks = int(os.environ.get("CUBEFIT_WORKER_MAXTASKS", "0"))  # 0 = no recycle
    ping_timeout = float(os.environ.get("CUBEFIT_POOL_PING_TIMEOUT", "5.0"))
    renew_each_epoch = os.environ.get("CUBEFIT_POOL_RENEW_EVERY_EPOCH", "0").lower() not in ("0","false","no","off")

    def _make_pool():
        return ctx.Pool(
            processes=nprocs,
            initializer=_worker_init,
            initargs=(int(cfg.blas_threads),),
            maxtasksperchild=(None if max_tasks <= 0 else max_tasks),
        )

    pool = _make_pool()

    print("[Kaczmarz-MP] Workers ready. Starting epochs…", file=sys.__stdout__, flush=True)

    try:
        for ep in range(cfg.epochs):
            # Ensure the pool is healthy (or renew it) before launching work
            if not _pool_ok(pool, timeout=ping_timeout) or renew_each_epoch:
                try:
                    pool.close()
                    pool.join()
                except Exception:
                    try:
                        pool.terminate()
                    except Exception:
                        pass
                pool = _make_pool()
            pbar = tqdm(total=len(s_ranges), desc=f"[Kaczmarz-MP] epoch {ep+1}/{cfg.epochs}",
                        mininterval=2.0, dynamic_ncols=True)
            pbar.refresh()

            for tile_idx, (s0, s1) in enumerate(s_ranges):
                Sblk = s1 - s0

                # ---------- Build residual R = Y - yhat ----------
                with open_h5(h5_path, role="reader") as f:
                    DC = f["/DataCube"]; M  = f["/HyperCube/models"]
                    _set_chunk_cache(M, cfg)

                    Y = np.asarray(DC[s0:s1, :], dtype=np.float64, order="C")
                    if keep_idx is not None:
                        Y = Y[:, keep_idx]

                    yhat = np.zeros((Sblk, Lk), dtype=np.float64)
                    for c in range(C):
                        A = np.asarray(M[s0:s1, c, :, :], dtype=np.float32, order="C")
                        if keep_idx is not None:
                            A = A[:, :, keep_idx]
                        xc = x_CP[c, :].astype(np.float64, copy=False)
                        for s in range(Sblk):
                            yhat[s, :] += xc @ A[s, :, :]
                    R = Y - yhat

                tracker.on_batch_rmse(float(np.sqrt(np.mean(R * R))))

                # Cosine LR decay across tiles
                if cfg.epochs >= 1:
                    frac = (tile_idx + 1) / len(s_ranges)
                    lr_tile = float(cfg.lr) * (0.5 + 0.5 * np.cos(np.pi * frac))
                else:
                    lr_tile = float(cfg.lr)

                # ---------- Workers ----------
                jobs = []
                for (c_start, c_stop) in bands:
                    x_band = x_CP[c_start:c_stop, :].copy()
                    w_band = None if w_full is None else w_full[c_start:c_stop].copy()
                    E_band = E_global[c_start:c_stop, :]  # (band_size,P)
                    jobs.append((
                        h5_path, s0, s1, keep_idx,
                        c_start, c_stop, x_band,
                        float(lr_tile), bool(cfg.project_nonneg),
                        R.copy(), w_band,
                        cfg.dset_slots, cfg.dset_bytes, cfg.dset_w0,
                        E_band, float(beta_blend),
                        w_lam_sqrt
                    ))

                results = pool.map(_worker_tile_job_with_R, jobs)

                # --------------- aggregate & tile backtracking ---------------
                R_delta_agg = np.zeros_like(R)
                band_updates = []
                cand_pairs = []
                upd_energy_sq_total = 0.0

                for item in results:
                    if isinstance(item, tuple) and len(item) == 3:
                        R_delta, dx_list, cands = item
                        if cands:
                            cand_pairs.extend(cands)
                    else:
                        R_delta, dx_list = item
                    R_delta_agg += R_delta
                    for tup in dx_list:
                        if len(tup) == 3:
                            c, dx_c, e2 = tup
                            upd_energy_sq_total += float(e2)
                        else:
                            c, dx_c = tup
                        band_updates.append((c, dx_c))

                bt_steps_tile  = int(np.max((0, int(os.environ.get("CUBEFIT_TILE_BT_STEPS", "6")))))
                bt_factor_tile = float(os.environ.get("CUBEFIT_TILE_BT_FACTOR", "0.5"))

                rmse_before = float(np.sqrt(np.mean(R * R)))

                alpha = 1.0
                rmse_after = float(np.sqrt(np.mean((R + alpha * R_delta_agg) ** 2)))
                for _ in range(bt_steps_tile):
                    if rmse_after < rmse_before:
                        break
                    alpha *= bt_factor_tile
                    if alpha <= 0.0:
                        break
                    rmse_after = float(np.sqrt(np.mean((R + alpha * R_delta_agg) ** 2)))

                if not np.isfinite(rmse_after):
                    alpha *= 0.0
                    rmse_after = rmse_before
                tracker.on_batch_rmse(rmse_after)

                # -------- GLOBAL trust region (use global energy & global ||Y||) --------
                if (upd_energy_sq_total > 0.0) and (Y_glob_norm > 0.0):
                    global_step_norm = float(np.sqrt(upd_energy_sq_total)) * alpha
                    cap = float(tau_global * Y_glob_norm)
                    if global_step_norm > cap:
                        alpha *= float(cap / np.max((1e-12, global_step_norm)))

                # ----------------- apply α ONCE to both x and R -----------------
                R += alpha * R_delta_agg
                for (c, dx_c) in band_updates:
                    x_CP[c, :] += alpha * dx_c
                    if cfg.project_nonneg:
                        np.maximum(x_CP[c, :], 0.0, out=x_CP[c, :])

                # ---------- optional ratio penalty ----------
                if have_ratio:
                    _ratio_update_in_place(x_CP)

                 # ---------- Optional: tile-local NNLS polish ----------
                nnls_enable = os.environ.get("CUBEFIT_NNLS_ENABLE", "1").lower() not in ("0","false","no","off")
                nnls_every = int(os.environ.get("CUBEFIT_NNLS_EVERY", "1"))
                nnls_max_cols = int(os.environ.get("CUBEFIT_NNLS_MAX_COLS", "256"))
                nnls_max_bytes = int(os.environ.get("CUBEFIT_NNLS_MAX_BYTES", str(1_000_000_000)))
                nnls_sub_L = int(os.environ.get("CUBEFIT_NNLS_SUB_L", "0"))  # 0==use all
                nnls_solver = os.environ.get("CUBEFIT_NNLS_SOLVER", "pg").lower()  # "pg"|"fista"|"nnls"|"lsq"
                nnls_max_iter = int(os.environ.get("CUBEFIT_NNLS_MAX_ITER", "200"))
                nnls_min_improve = float(os.environ.get("CUBEFIT_NNLS_MIN_IMPROVE", "0.999"))
                nnls_l2 = float(os.environ.get("CUBEFIT_NNLS_L2", "0.0")) # small ridge, e.g. 1e-8

                do_nnls = (nnls_enable and ((tile_idx % nnls_every) == 0) and (len(cand_pairs) > 0))

                if do_nnls:
                    # Deduplicate by best score per global index
                    idxs = np.asarray([int(t[0]) for t in cand_pairs], dtype=np.int64)
                    scrs = np.asarray([float(t[1]) for t in cand_pairs], dtype=np.float64)
                    best = {}
                    for i, sc in zip(idxs, scrs):
                        if (i not in best) or (sc > best[i]):
                            best[i] = sc
                    uniq_idx = np.fromiter(best.keys(), dtype=np.int64)
                    uniq_scr = np.fromiter(best.values(), dtype=np.float64)

                    order = np.argsort(uniq_scr)[::-1]
                    uniq_idx = uniq_idx[order]

                    # Subsample wavelengths to cap rows
                    Lk_loc = int(R.shape[1])
                    if (nnls_sub_L > 0) and (Lk_loc > nnls_sub_L):
                        rng = np.random.default_rng(12345 + tile_idx)
                        lam_sel = np.sort(rng.choice(Lk_loc, size=int(nnls_sub_L), replace=False))
                    else:
                        lam_sel = np.arange(Lk_loc, dtype=np.int64)

                    rows = int(Sblk * lam_sel.size)

                    # Cap columns by memory
                    bytes_per_col = int(rows * 8)  # float64
                    cap_by_mem = int(np.max((1, nnls_max_bytes // np.max((1, bytes_per_col)))))
                    K_use = int(np.min((uniq_idx.size, nnls_max_cols, cap_by_mem)))

                    if (K_use >= 2) and (rows >= 2):
                        W_idx = uniq_idx[:K_use]

                        # λ-weights for this subsample (use preloaded w_lam_sqrt if available)
                        if w_lam_sqrt is not None:
                            wlam_sel = w_lam_sqrt[lam_sel].astype(np.float64, copy=False)  # (|lam_sel|,)
                        else:
                            wlam_sel = np.ones(lam_sel.size, dtype=np.float64)
                        # Row weights replicated across spaxels
                        sqrt_w_rows = np.tile(wlam_sel, Sblk).astype(np.float64)  # (rows,)

                        # Build B and xW on the subsample
                        with open_h5(h5_path, role="reader") as f:
                            M = f["/HyperCube/models"]
                            PP = int(M.shape[2])
                            groups = {}
                            for j, g in enumerate(W_idx):
                                cc = int(g // PP); pp = int(g % PP)
                                groups.setdefault(cc, []).append((pp, j))

                            B = np.empty((rows, K_use), dtype=np.float64)
                            xW = np.zeros((K_use,), dtype=np.float64)

                            for cc, plist in groups.items():
                                A_c = np.asarray(M[s0:s1, cc, :, :], dtype=np.float32, order="C")
                                if keep_idx is not None:
                                    A_c = A_c[:, :, keep_idx] # (Sblk,P,Lk)
                                A_c = A_c[:, :, lam_sel] # (Sblk,P,|lam_sel|)
                                for pp, j in plist:
                                    col = A_c[:, int(pp), :].astype(np.float64, copy=False).reshape(rows, order="C")
                                    B[:, int(j)] = col
                                    xW[int(j)]   = float(x_CP[int(cc), int(pp)])

                        # Subsample residual and RHS (unweighted first)
                        r_sub = R[:, lam_sel].reshape(rows, order="C")
                        y_rhs = B @ xW + r_sub

                        # Weight the system: minimize || sqrt(W) * (B x - y) ||_2
                        B_w     = B * sqrt_w_rows[:, None]
                        y_rhs_w = y_rhs * sqrt_w_rows

                        xW_new = None
                        if nnls_solver in ("pg", "fista"):
                            # Column-normalize (improves conditioning)
                            col_norm = np.linalg.norm(B_w, axis=0)
                            col_norm = np.where(col_norm > 0.0, col_norm, 1.0)
                            Bn = B_w / col_norm
                            x  = xW / col_norm

                            # FISTA with backtracking (falls back to PGD if t stays ~1)
                            L_est = float(np.linalg.norm(Bn, ord=2)**2 + nnls_l2) if K_use <= 2048 else float((Bn**2).sum(axis=0).max() + nnls_l2)
                            t = 1.0
                            z = np.maximum(0.0, x.copy())
                            x_old = z.copy()

                            def _f_and_grad(zvec):
                                r = Bn @ zvec - y_rhs_w
                                f = 0.5 * float(r @ r) + 0.5 * nnls_l2 * float(zvec @ zvec)
                                g = Bn.T @ r + nnls_l2 * zvec
                                return f, g, r

                            fz, gz, rz = _f_and_grad(z)
                            step = 1.0 / max(L_est, 1e-6)

                            for _it in range(nnls_max_iter):
                                # Backtracking Armijo
                                while True:
                                    x_try = z - step * gz
                                    np.maximum(x_try, 0.0, out=x_try)
                                    r_try = Bn @ x_try - y_rhs_w
                                    f_try = 0.5 * float(r_try @ r_try) + 0.5 * nnls_l2 * float(x_try @ x_try)
                                    # sufficient decrease
                                    if f_try <= fz - 1e-4 * step * float(gz @ gz) or step < 1e-12:
                                        break
                                    step *= 0.5
                                # FISTA momentum
                                t_new = 0.5 * (1.0 + math.sqrt(1.0 + 4.0 * t * t))
                                z = x_try + ((t - 1.0) / t_new) * (x_try - x_old)
                                x_old = x_try
                                t = t_new
                                fz, gz, rz = _f_and_grad(z)

                                # tiny early-stop on relative residual change
                                if float(rz @ rz) < 1e-12:
                                    break

                            xW_new = x_try * col_norm

                        elif nnls_solver == "lsq":
                            try:
                                from scipy.optimize import lsq_linear
                                res = lsq_linear(B_w, y_rhs_w, bounds=(0.0, np.inf),
                                                 method="trf", max_iter=nnls_max_iter, verbose=0)
                                xW_new = np.maximum(0.0, res.x)
                            except Exception:
                                xW_new = None
                        elif nnls_solver == "nnls":
                            try:
                                from scipy.optimize import nnls as _scipy_nnls
                                xW_new, _ = _scipy_nnls(B_w, y_rhs_w)
                            except Exception:
                                xW_new = None

                        # Commit only if we actually solved something and it helped enough
                        if xW_new is not None:
                            dxW = xW_new - xW
                            if np.any(dxW != 0.0):
                                # Weighted RMSE accept criterion
                                def _wrmse(vec_1d: np.ndarray) -> float:
                                    w2 = sqrt_w_rows * sqrt_w_rows
                                    num = float(np.dot(w2, vec_1d * vec_1d))
                                    den = float(np.sum(w2)) or 1.0
                                    return math.sqrt(num / den)

                                r_after = r_sub - (B @ dxW)  # NOTE: unweighted update to R
                                rmse_before = _wrmse(r_sub)
                                rmse_after  = _wrmse(r_after)

                                if (rmse_after / max(1e-12, rmse_before)) < nnls_min_improve:
                                    for j, g in enumerate(W_idx):
                                        cc = int(g // P); pp = int(g % P)
                                        x_CP[cc, pp] = float(max(0.0, x_CP[cc, pp] + dxW[j]))
                                    # Update residual in the *unweighted* space
                                    R[:, lam_sel] = r_after.reshape(Sblk, lam_sel.size, order="C")
                # ---------- end NNLS polish ----------

                tracker.on_progress(
                    epoch=ep + 1,
                    spax_done=tile_idx + 1,
                    spax_total=len(s_ranges),
                    rmse_ewma=None  # or your running EWMA if you have one handy
                )

                tracker.maybe_snapshot_x(x_CP, epoch=ep, rmse=rmse_after)

                pbar.update(1)
                pbar.refresh()

            pbar.close()
            # print(f"[Kaczmarz-MP] epoch {ep+1}/{cfg.epochs} tiles complete; de-correlating...", flush=True)

            # # ---- tiny cross-component decorrelation (once per epoch) ----
            # gamma = float(os.environ.get("CUBEFIT_DECORR_GAMMA", "1e-4"))
            # if gamma > 0.0:
            #     with _single_blas_thread():
            #         # x_CP shape (C, P)
            #         X = x_CP.astype(np.float64, copy=False)
            #         rnorm = np.linalg.norm(X, axis=1, keepdims=True) + 1e-30 # (C,1)
            #         U = X / rnorm # row-whitened
            #         G = U @ U.T # (C,C) Gram
            #         D = U @ (G - np.eye(U.shape[0])) # grad in U
            #         X -= gamma * (D * rnorm) # scale back
            #         if cfg.project_nonneg:
            #             np.maximum(X, 0.0, out=X)
            #         x_CP[:] = X
            #         del X, U, G, D, rnorm
            
            print(f"[Kaczmarz-MP] epoch {ep+1}/{cfg.epochs} snapshotting...", flush=True)
            # Do not block here:
            try:
                tracker.on_epoch_end(ep + 1, dict(), block=False)
            except Exception:
                pass

            # Best-effort snapshot (won't block):
            try:
                tracker.maybe_snapshot_x(x_CP, epoch=ep+1, rmse=rmse_after, force=True)
            except Exception:
                pass

            print(f"[Kaczmarz-MP] epoch {ep+1}/{cfg.epochs} housekeeping done.", flush=True)

        elapsed = time.perf_counter() - t0
        return x_CP.ravel(order="C"), dict(epochs=cfg.epochs, elapsed_sec=elapsed)
    finally:
        pool.close()
        pool.join()

# ------------------------------------------------------------------------------

def probe_kaczmarz_tile(
    h5_path: str,
    s0: int | None = None,
    s1: int | None = None,
    c: int | None = None,
    lr: float = 0.25,
    x_source: str = "auto",   # "auto" | "zeros"
    project_nonneg: bool = True,
):

    bt_steps   = int(np.max((0, int(os.environ.get("CUBEFIT_BT_STEPS", "3")))))
    bt_factor  = float(os.environ.get("CUBEFIT_BT_FACTOR", "0.5"))
    tau_trust  = float(os.environ.get("CUBEFIT_TRUST_TAU", "0.7"))
    eps        = float(os.environ.get("CUBEFIT_EPS", "1e-12"))
    rel_zero   = float(os.environ.get("CUBEFIT_ZERO_COL_REL", "1e-12"))
    abs_zero   = float(os.environ.get("CUBEFIT_ZERO_COL_ABS", "1e-24"))
    tau_global = float(os.environ.get("CUBEFIT_GLOBAL_TAU", "0.5"))
    beta_blend = float(os.environ.get("CUBEFIT_GLOBAL_ENERGY_BLEND", "1e-3"))

    with h5py.File(h5_path, "r") as f:
        M  = f["/HyperCube/models"]  # (S,C,P,L)
        DC = f["/DataCube"]          # (S,L)
        S, Ctot, P, L = map(int, M.shape)
        chunks = M.chunks or (S, 1, P, L)
        S_chunk = int(chunks[0])

        if s0 is None or s1 is None:
            s0 = 0
            s1 = int(np.min((S, S_chunk)))
        if c is None:
            c = int(Ctot // 2)

        keep_idx = None
        if "/Mask" in f:
            m = np.asarray(f["/Mask"][...], bool).ravel()
            keep_idx = np.flatnonzero(m)
        Lk = int(L if keep_idx is None else keep_idx.size)

        # x source
        if x_source == "auto" and "/X_global" in f:
            x1d = np.asarray(f["/X_global"][...], np.float64, order="C")
            x_CP = x1d.reshape(Ctot, P)
        else:
            x_CP = np.zeros((Ctot, P), np.float64)

        # Y (tile), global ||Y|| for global cap
        Y = np.asarray(DC[s0:s1, :], np.float64, order="C")
        if keep_idx is not None:
            Y = Y[:, keep_idx]  # (Sblk, Lk)
        Sblk = int(s1 - s0)

        # global ||Y||
        Yglob2 = 0.0
        for t0 in range(0, S, S_chunk):
            t1 = int(np.min((S, t0 + S_chunk)))
            Yt = np.asarray(DC[t0:t1, :], np.float64, order="C")
            if keep_idx is not None:
                Yt = Yt[:, keep_idx]
            Yglob2 += float(np.sum(Yt * Yt))
        Y_glob_norm = float(np.sqrt(Yglob2))

        # yhat (tile) exactly like the solver
        yhat = np.zeros((Sblk, Lk), np.float64)
        for cc in range(Ctot):
            A_cc = np.asarray(M[s0:s1, cc, :, :], np.float32, order="C")
            if keep_idx is not None:
                A_cc = A_cc[:, :, keep_idx]
            xc = x_CP[cc, :].astype(np.float64, copy=False)
            for s in range(Sblk):
                yhat[s, :] += xc @ A_cc[s, :, :]

        R = Y - yhat

        # ---- worker-like band update on component c ----
        A = np.asarray(M[s0:s1, c, :, :], np.float32, order="C")
        if keep_idx is not None:
            A = A[:, :, keep_idx]  # (Sblk, P, Lk)

        # sanitize
        badR = ~np.isfinite(R); R[badR] = 0.0
        badA = ~np.isfinite(A); A[badA] = 0.0

        # gradient
        g = np.zeros((P,), np.float64)
        for s in range(Sblk):
            g += A[s, :, :].astype(np.float64, copy=False) @ R[s, :]

        # local per-column denom
        col_denom = np.sum(np.square(A, dtype=np.float64), axis=(0, 2))  # (P,)

        # freeze near-zero columns (tile-local)
        med_energy = float(np.median(col_denom[col_denom > 0])) if np.any(col_denom > 0) else 0.0
        tiny_col = np.max((abs_zero, rel_zero * med_energy))
        freeze = col_denom <= tiny_col
        if np.any(freeze):
            g[freeze] = 0.0
            col_denom = np.where(freeze, np.inf, col_denom)

        # --- NEW: blend with global column energy for this component
        E_global = read_global_column_energy(h5_path)  # (C,P)
        Eg_row = np.asarray(E_global[int(c), :], np.float64) # (P,)
        col_denom = np.maximum(col_denom, float(beta_blend) * Eg_row)

        invD = 1.0 / np.maximum(col_denom, eps)
        dx_c = float(lr) * (g * invD)  # (P,)

        # ΔR for alpha=1
        R_delta = np.zeros((Sblk, Lk), np.float64)
        for s in range(Sblk):
            R_delta[s, :] -= (A[s, :, :].astype(np.float64, copy=False).T @ dx_c)

        # trust region (tile)
        rd = float(np.linalg.norm(R_delta))
        if rd > 0.0:
            rn = float(np.linalg.norm(R))
            alpha_max = float(np.minimum(1.0, (tau_trust * rn) / rd))
        else:
            alpha_max = 1.0

        # backtracking
        alpha = alpha_max
        rmse_before = float(np.sqrt(np.mean(R * R)))
        rmse_after  = float(np.sqrt(np.mean((R + alpha * R_delta) ** 2)))
        if not (rmse_after < rmse_before):
            a = alpha
            for _ in range(bt_steps):
                a *= bt_factor
                if a <= 0.0:
                    break
                rmse_after = float(np.sqrt(np.mean((R + a * R_delta) ** 2)))
                if rmse_after < rmse_before:
                    alpha = a
                    break
            else:
                alpha = a  # may be ~0

        # ---- NEW: global cap based on global energy and ||Y||
        upd_energy_sq = float(np.sum((dx_c.astype(np.float64) ** 2) * Eg_row))
        if (upd_energy_sq > 0.0) and (Y_glob_norm > 0.0):
            step_norm_global = float(np.sqrt(upd_energy_sq)) * alpha
            cap = float(tau_global * Y_glob_norm)
            if step_norm_global > cap:
                alpha *= float(np.minimum(1.0, cap / np.maximum(1e-12, step_norm_global)))

        dx_c *= alpha
        if project_nonneg:
            over_neg = dx_c < -x_CP[c, :]
            if np.any(over_neg):
                dx_c[over_neg] = -x_CP[c, :][over_neg]
                R_delta.fill(0.0)
                for s in range(Sblk):
                    R_delta[s, :] -= (A[s, :, :].astype(np.float64, copy=False).T @ dx_c)
        else:
            if alpha != 1.0:
                R_delta *= alpha

        R_after = R + R_delta
        yhat_norm = float(np.linalg.norm(yhat))
        yhat_next_norm = float(np.linalg.norm(yhat - R_delta))  # since R_after = Y - yhat_next

        out = {
            "rmse_before": float(np.sqrt(np.mean(R * R))),
            "rmse_after":  float(np.sqrt(np.mean(R_after * R_after))),
            "y_norm":      float(np.linalg.norm(Y)),
            "yhat_norm":   yhat_norm,
            "yhat_next_norm": yhat_next_norm,
            "g_norm":      float(np.linalg.norm(g)),
            "dx_norm":     float(np.linalg.norm(dx_c)),
            "global_upd_norm": float(np.sqrt(np.maximum(0.0, upd_energy_sq)) * alpha),
            "Y_glob_norm": Y_glob_norm,
            "Sblk":        Sblk,
            "Lk":          Lk,
            "c":           int(c),
            "alpha":       float(alpha),
            "frozen_cols": int(np.count_nonzero(freeze)),
        }
        print("[Probe]", out)
        return out
