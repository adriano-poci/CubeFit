...
def streamActiveSetNNLS(
    ...
    *,
    ...
    final_active_idx = np.zeros((0,), dtype=np.int64)
    final_S_active = np.zeros((0,), dtype=np.float64)
    final_ATA_sub = None
    final_ATy_sub = None
    final_z_sub = None
    ...
    def _orbit_mass_and_deficit(z_vec: np.ndarray):
        """
        Compute current per-orbit masses and deficits relative to the
        current orbit prior scale.
        """
    ...
    for it in range(start_iter, max_iter):
        ...
        final_active_idx = active_idx.copy()
        final_S_active = S_active.copy()
        final_ATA_sub = ATA_sub_reg.copy()
        final_ATy_sub = ATy_sub.copy()
        final_z_sub = z_sub.copy()
